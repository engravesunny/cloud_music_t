<template>
  <div class="songListPage_container unselectable">
      ...
      <div v-loading="!songList_info[0]" class="other_info">
        ...
        <div v-if="songList_info[0]" class="found_info">
          ...
          <div class="found_name">
            {{
              songList_info[0]?.creator?.nickname ||
              songList_info[0]?.artist?.name
            }}
          </div>
          <!-- 创建者昵称 -->

          <!-- 创建日期 -->
          <div class="found_date">
            {{
              formatDate(
                songList_info[0]?.createTime || songList_info[0]?.publishTime,
              ) + "创建"
            }}
          </div>
          <!-- 创建日期 -->
        ...
        <div v-if="songList_info[0]" class="rest_info">
          <!-- 歌曲总数 -->
          <div class="songs_total iconfont">
            &#xea86; {{ songList_info[0].trackCount || albumSongList.length }}
          </div>
          <!-- 歌曲总数 -->

          <!-- 播放量 -->
          <div class="songs_playtotal iconfont">
            &#xe600; {{ songList_info[0].playCount || "" }}
          </div>
          <!-- 播放量 -->
        </div>
        <!-- 其他信息 -->
        ...
        <div v-if="songList_info[0]" class="playall_btn">
          <el-button class="iconfont btn" type="primary" @click="playAllFn">
            &#xe87c;
          </el-button>
    ...
    <!-- 歌曲列表 -->
    <singleSong
      v-if="songList.length || albumSongList.length"
      v-loading="!songList.length && !albumSongList.length"
      :song-total="0"
      :current-pages="0"
      :result="albumSongList.length ? albumSongList : songList"
    />
    <!-- 歌曲列表 -->
    <!-- 加载状态 -->
    <p v-if="isloading && !albumSongList.length" class="loading">Loading...</p>
    <p v-if="finished || albumSongList.length" class="loading">已经没有了哦</p>
    <div ref="observerDom" />
  </div>
</template>
