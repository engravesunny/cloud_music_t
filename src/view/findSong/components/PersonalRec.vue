<template>
        ...
        >
          ...
          <div class="left-img" fit="cover">
            <img
              ...
              alt="每日歌曲推荐"
              @load="handleLoad"
              :src="http2Https(item.coverSrc + '?params=150y150')"
            />
          </div>
          ...
          <div class="playBtn iconfont" @click.stop="handlePlayClick(item)">
            &#xe87c;
          </div>
        </div>
      </div>
  ...
  <div class="suggest_playlist">
    <suggestList
      :suggestLists="dailyList"
      :title="dailyListTitle"
    ></suggestList>
  </div>
  <!-- 每日推荐 -->
  ...
  <div class="suggest_playlist">
    <suggestList
      :suggestLists="highPlayList"
      :title="highPlayListTitle"
    ></suggestList>
  </div>
  <!-- 精品歌单 -->
</template>
...
const getCover = async () => {
  try {
    ...
    if (data1.code === 200 && data2.code === 200) {
      ...
      // 设置描述
      section1List[1].des =
        songs1.songs[0].name + "-" + songs1.songs[0].ar[0].name;
      section1List[2].des =
        songs2.songs[0].name + "-" + songs2.songs[0].ar[0].name;
      // 设置歌曲列表
