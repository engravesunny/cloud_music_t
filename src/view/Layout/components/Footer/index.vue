<template>
  <div class="footer unselectable">
    ...
    <div v-if="songState.name" class="songInfo">
      <div class="songName shenglue" :title="songState.name">
        {{ songState.name }}
      </div>
      <div class="songAr shenglue" :title="mulArShow(songState.ar)">
        {{ mulArShow(songState.ar) }}
      </div>
    </div>
    <!-- 歌曲名称 -->
    ...
    <div class="timeProgress">
      ...
      <div class="progress">
        <el-slider
          v-model="playedProgress"
          :show-tooltip="false"
          @change="changePlayedTimeLast"
    ...
    <div v-if="!isFM" class="playMode" @click="changePlayMode">
      <div v-if="songState.playMode === 0" class="seqPlay iconfont">
        &#xea6f;
      </div>
      <div v-if="songState.playMode === 1" class="loopPlay iconfont">
        &#xe66c;
      </div>
      <div v-if="songState.playMode === 2" class="singlePlay iconfont">
        &#xe66d;
      </div>
      <div v-if="songState.playMode === 3" class="randomPlay iconfont">
        &#xe66b;
      </div>
    </div>
    <!-- 播放模式 -->
    ...
    <!-- 音量 -->
    <volum v-if="isChangingVolume" class="volumeOption"></volum>
    <div class="volume iconfont"><span class="icon">&#xe605;</span></div>
    <!-- 音量 -->

    <!-- 播放列表 -->
    <div
      v-if="showSongList && !songState.FMMode"
      class="close"
      @click="showSongList = false"
    >
      <songList v-if="showSongList"></songList>
    </div>
    <div v-if="!isFM" class="playList iconfont" @click="playListShow">
      <span class="icon">&#xe62d;</span>
    </div>
    <div v-else class="playList iconfont" @click="playListShow">
      <span class="icon"></span>
    </div>
    <!-- 播放列表 -->
  </div>
  <song-display
    :next-song="SongEnd"
    :play-it="changePlayingState"
    :audio-dom="audioDom"
    :show="showSongDisplay"
  ></song-display>
</template>
